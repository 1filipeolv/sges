...
function NavItem({ item, user }) {
  ...
  return (
    ...
    >
      {({ isActive }) => <><Icon size={15} strokeWidth={isActive ? 2.5 : 2} />{item.label}</>}
    </NavLink>
  );
}
...
export default function Layout({ children }) {
  ...
  return (
    <div style={{ display: 'flex', height: '100vh', overflow: 'hidden', fontFamily: 'Inter, system-ui, sans-serif' }}>
      <aside style={{ width: 228, background: '#fff', borderRight: '1px solid #E4E4E7', display: 'flex', flexDirection: 'column', flexShrink: 0 }}>
